// Inject Xiumi Styles
var BLANK = '<section style="font-size: inherit;color: inherit;line-height: inherit;margin: 1.5em 8px;"></section>'
...
var WxRenderer = function (opts) {
  ...
  this.buildFootnotes = function () {
    ...
    return BLANK + H2_PREFIX + 'Reference' + H2_POSTFIX +
			'<p ' + S('footnotes') + '>'  + footnoteArray.join('\n') + '</p>'
  }
  ...
  this.getRenderer = function () {
    ...
    renderer.heading = function (text, level) {
			if (level == 1) {
				...
				if (_this.firstTitle) {
					_this.firstTitle = false;
					return H2_PREFIX + text + H2_POSTFIX
				} else {
					return BLANK + H2_PREFIX + text + H2_POSTFIX
				}
		  } else if (level == 3) {
    ...
    renderer.paragraph = function (text) {
			return '<p style="box-sizing: border-box;">' + text + '</p>'
			//return '<p ' + S('p') + '>' + text + '</p>'
    }
    ...
    renderer.hr = function(){
      return '<hr style="font-size: inherit;color: inherit;line-height: inherit;height: 1px;margin-top: 1.5rem;margin-bottom: 1.5rem;border-style: dashed none none;border-top-color: rgb(165, 165, 165);">';
    }
    return renderer
  }
}
